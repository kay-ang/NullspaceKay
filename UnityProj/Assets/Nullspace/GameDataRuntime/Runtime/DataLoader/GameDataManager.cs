...
namespace Nullspace
{
    ...
    public partial class GameDataManager
    {
                    ...
                    if (item.Namespace == GameDataNamespaceName)
                    {
                        ...
                        while (type != null)
                        {
                            try
                            {
                                ...
                                {
                                    FieldInfo field = item.GetField(FileURL);
                                    if (field != null)
                                    {
                                        string fileUrl = field.GetValue(null) as string;
                                        string[] strs = fileUrl.Split(FileUrlSplit);
                            ...
                            {
                                DebugUtils.Log(InfoType.Error, "%s", e.Message);
                            }
                        }
                    }
                }
        ...
        {
            string filePath = string.Format("{0}/{1}.xml", FileDir, fileName);
            if (IsAssetBundle)
            {
